...
export function ImageProperties({ el, onChange, onOpenPicker }: Props) {
  ...
  const mainAsset                = src.type === 'asset' ? getAsset(src.assetId) : null
  const isMainMissing            = src.type === 'asset' && !mainAsset
  const placeholderAsset         = src.type === 'binding' && src.placeholder
    ? getAsset(src.placeholder.assetId) : null
  const isPlaceholderMissing     = src.type === 'binding' && src.placeholder && !placeholderAsset

  const switchToBinding = async () => {
    if (src.type === 'asset' && src.assetId) {
      const ok = await confirm({
        title: 'Switch to column binding?',
        message: 'The currently set image will be removed.',
        confirmLabel: 'Switch', cancelLabel: 'Keep asset', variant: 'danger',
      })
      if (!ok) return
    }
    onChange({ props: { ...el.props, src: { type: 'binding', column: '', placeholder: undefined } } })
  }
  ...
  const switchToAsset = async () => {
    if (src.type === 'binding' && (src.column || src.placeholder)) {
      const ok = await confirm({
        ...
        confirmLabel: 'Switch', cancelLabel: 'Keep binding', variant: 'danger',
      })
      if (!ok) return
    }
    ...
    <>
      <div className={styles.section}>
        ...
        {!isBinding && (
          <div className={styles.imageSourceBlock}>
            {isMainMissing ? (
              <div className={styles.missingAssetBlock}>
                <div className={styles.missingAssetInfo}>
                  ...
                  <div>
                    ...
                    <p className={styles.missingDesc}>This image is not in your library.</p>
                    <code className={styles.missingId}>{(src as any).assetName}</code>
                  </div>
                </div>
                <div className={styles.imageSourceBtns}>
                  <button className={styles.imageSourceBtn} onClick={() => onOpenPicker('main')}>Import image</button>
                  <button
                    className={`${styles.imageSourceBtn} ${styles.imageSourceBtnDanger}`}
                    onClick={() => onChange({ props: { ...el.props, src: { type: 'none' } } })}
                  >Clear</button>
                </div>
              </div>
            ) : mainAsset ? (
              <div className={styles.imageSourceSet}>
                <span className={styles.imageAssetName} title={mainAsset.name}>{mainAsset.name}</span>
                <span className={styles.imageAssetDims}>{mainAsset.width} × {mainAsset.height}px</span>
                <div className={styles.imageSourceBtns}>
                  <button className={styles.imageSourceBtn} onClick={() => onOpenPicker('main')}>Change</button>
                  <button
                    className={`${styles.imageSourceBtn} ${styles.imageSourceBtnDanger}`}
                    onClick={() => onChange({ props: { ...el.props, src: { type: 'none' } } })}
                  >Remove</button>
              ...
              <button className={styles.setImageBtn} onClick={() => onOpenPicker('main')}>+ Set image</button>
            )}
          </div>
        )}
          ...
          <div className={styles.bindingBlock}>
            <div className={styles.bindingFieldGroup}>
              ...
              <input
                ...
                value={src.column}
                onChange={(e) => onChange({ props: { ...el.props, src: { ...src, column: e.target.value } } })}
              />
            </div>
            ...
            <div className={styles.bindingFieldGroup}>
              ...
              {isPlaceholderMissing ? (
                <div className={styles.missingAssetBlock}>
                  <div className={styles.imageSourceBtns}>
                    <button className={styles.imageSourceBtn} onClick={() => onOpenPicker('placeholder')}>Import image</button>
                    <button
                      className={`${styles.imageSourceBtn} ${styles.imageSourceBtnDanger}`}
                      onClick={() => onChange({ props: { ...el.props, src: { ...src, placeholder: undefined } } })}
                    >Clear</button>
                  </div>
                </div>
              ) : placeholderAsset ? (
                <div className={styles.imageSourceSet}>
                  ...
                  <div className={styles.imageSourceBtns}>
                    <button className={styles.imageSourceBtn} onClick={() => onOpenPicker('placeholder')}>Change</button>
                    <button
                      className={`${styles.imageSourceBtn} ${styles.imageSourceBtnDanger}`}
                      onClick={() => onChange({ props: { ...el.props, src: { ...src, placeholder: undefined } } })}
                    >Remove</button>
        ...
        <div className={styles.propRow} style={{ marginTop: 8 }}>
          ...
          <div className={styles.propControl}>
            <select
              ...
              value={el.props.fit}
              onChange={(e) => onChange({ props: { ...el.props, fit: e.target.value as any } })}
            >
        ...
        <PropRow label="Radius">
          <NumericInput
            value={el.props.cornerRadius ?? 0}
            min={0} max={Math.min(el.width, el.height) / 2}
